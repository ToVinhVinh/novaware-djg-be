"""
Graph Convolutional Network (GCN) model implementation using PyTorch Geometric.
Implements standard GCN for collaborative filtering with feature transformation and ReLU activation.

Key differences from LightGCN:
- Uses feature transformation (linear layers) in each GCN layer
- Uses ReLU activation function
- Uses only final layer's embeddings (not averaging all layers)
"""
...
class GNNConv(MessagePassing):
    ...
    Implements standard GCN with feature transformation and ReLU activation.
    ...
    def forward(self, x, edge_index):
        ...
        # Propagate messages: D^(-1/2) A D^(-1/2) H^(l) W^(l)
    ...
    def message(self, x_j, norm):
        """Message function: normalized neighbor features."""
        return norm.view(-1, 1) * x_j


class GNN(nn.Module):
    """
    Graph Convolutional Network (GCN) model for collaborative filtering via user-item interaction graph.
    
    Architecture:
    - User and Product embeddings (initial features)
    - Multiple GCN layers with feature transformation and ReLU activation
    - Final embeddings from the last layer (standard GCN approach)
    
    Unlike LightGCN, GCN uses:
    - Feature transformation (linear layers) in each GCN layer
    - ReLU activation function
    - Only the final layer's embeddings (not averaging all layers)
    """
    ...
    def __init__(
        ...
        # Initialize embeddings (initial node features)
        ...
        # GCN layers with feature transformation
        # Each layer transforms features from embedding_dim to embedding_dim
        self.convs = nn.ModuleList([
            GNNConv(embedding_dim, embedding_dim) for _ in range(num_layers)
        ])
    ...
    def forward(self, edge_index: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Standard GCN approach: Use only the final layer's embeddings.
        Unlike LightGCN which averages all layers, GCN uses H^(L) where L is the last layer.
        
        Args:
            edge_index: Edge indices [2, num_edges] for user-product bipartite graph
            
        Returns:
            Tuple of (user_embeddings, product_embeddings) from the final GCN layer
        """
        # Get initial embeddings (H^(0))
        user_emb = self.user_embedding.weight
        product_emb = self.product_embedding.weight
        
        # Concatenate user and product embeddings
        x = torch.cat([user_emb, product_emb], dim=0)
        
        # Message passing through GCN layers
        # Each layer: H^(l+1) = ReLU(D^(-1/2) A D^(-1/2) H^(l) W^(l))
        for conv in self.convs:
            x = conv(x, edge_index)
        
        # Use final layer's embeddings (standard GCN approach)
        # Unlike LightGCN, we don't average all layers
        final_emb = x
...
class HeterogeneousGNN(nn.Module):
    """
    Heterogeneous Graph Convolutional Network (GCN) for multi-type nodes.
    Supports User ↔ Product ↔ Category ↔ Color ↔ Style relationships.
    
    Uses standard GCN layers with feature transformation and ReLU activation.
    """
    ...
    def forward(self, edge_index_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if 'user_product' in edge_index_dict:
            ...
            x = torch.cat([embeddings['user'], embeddings['product']], dim=0)
            
            # Message passing through GCN layers
            # Standard GCN: use only final layer's embeddings
            for conv in self.convs:
                x = conv(x, edge_index)
            
            # Use final layer's embeddings (GCN approach, not averaging)
            final_emb = x
