...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument("--user-id", required=True, help="Mongo ObjectId của user")
        parser.add_argument("--product-id", required=True, help="Mongo ObjectId của current product")
        parser.add_argument("--top-k-personal", type=int, default=5)
        parser.add_argument("--top-k-outfit", type=int, default=4)
        parser.add_argument("--outfile", default="gnn_mongo_result.json")
    ...
    def handle(self, *args, **options) -> str | None:
        ...
        payload: dict[str, Any] = recommend_gnn_mongo(
            user_id=user_id,
